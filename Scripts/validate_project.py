...
def validate_docs() -> CheckResult:
    ...
    require("Status: complete after local validation, remote full Xcode CI, and iOS Simulator launch smoke." in audit, "completion audit tracks complete delivery status")
    require("Remote GitHub Actions `iOS CI` runs the full Xcode gate" in audit, "completion audit tracks remote full Xcode CI gate")
    require("GitHub Actions `iOS CI` passes on `main`, including `make validate-ios`." in audit, "completion audit includes remote CI completion evidence")
    require("launches `com.codex.minidiablo`" in audit, "completion audit includes simulator launch smoke evidence")
    require("Chapter One, Survival, and Offline flows are covered by core smoke, XCTest, structural validation, and documented playtest paths." in audit, "completion audit maps playtest flows to validation evidence")
